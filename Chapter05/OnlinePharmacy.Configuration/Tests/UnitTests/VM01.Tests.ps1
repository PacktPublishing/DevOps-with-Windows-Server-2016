  <#
    Purpose:
        Verify that virtual machine VM01 is provisioned and configured appropriately.
...
#>
 ...
 Describe "web front end Virtual Machines vm01" {
   BeforeAll {
     ...
     $vm = Get-AzureRmVM -ResourceGroupName $resourceGroupName -Name vm1
     ...
     It "Virtual machine is attached to NIC " { 
         $vm.NetworkInterfaceIDs[0].Contains("$nicNameforVM1")| should be $true
     }
